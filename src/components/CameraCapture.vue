...
async function refreshCameraList() {
  ...
  const opts = devices
    ...
    .map((d, idx) => ({ deviceId: d.deviceId, label: d.label || `摄像头 ${idx + 1}` }))
  cameras.value = opts
  if (!selectedDeviceId.value && opts.length > 0) selectedDeviceId.value = opts[0].deviceId
}
...
async function start() {
  ...
  if (!window.isSecureContext) {
    emit('error', '当前不是安全上下文：摄像头只允许在 https 或 localhost 下使用。')
    return
  }
  ...
  try {
    ...
    stream = await navigator.mediaDevices.getUserMedia({ audio: false, video: buildConstraints() })
    // 读取真实 facingMode（iPad 上用 deviceId 选择时，UI facingMode 可能不等于实际）
    try {
      const track = stream.getVideoTracks()[0]
      const fm = (track?.getSettings?.() as any)?.facingMode as FacingMode | undefined
      if (fm === 'user' || fm === 'environment') actualFacingMode.value = fm
...
async function captureFrame(opts?: { updatePreview?: boolean; quality?: number }) {
  const v = videoEl.value
...
async function capture() {
  try {
    const { blob, dataUrl } = await captureFrame({ updatePreview: true, quality: 0.92 })
...
<template>
  <div class="wrap">
    <div class="controls">
      ...
      <label class="field">
        <span>摄像头</span>
        <select v-model="selectedDeviceId" :disabled="isRunning || cameras.length === 0">
      ...
      <div class="actions">
        <button class="btn primary" :disabled="!canStart" @click="start">开始</button>
        <button class="btn" :disabled="!isRunning" @click="stopStream">停止</button>
        <button class="btn" :disabled="!canCapture" @click="capture">拍照</button>
        <button class="btn" :disabled="!previewUrl" @click="clearPreview">清除预览</button>
      </div>
    </div>
