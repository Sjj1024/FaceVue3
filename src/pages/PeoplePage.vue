...
async function refresh() {
  ...
  try {
    ...
    if (!resp.ok) {
      error.value = data?.detail ? String(data.detail) : `请求失败：HTTP ${resp.status}`
      items.value = []
      return
    }
    items.value = Array.isArray(data?.items) ? (data.items as PersonItem[]) : []
  } catch (e) {
...
async function deletePerson(p: PersonItem) {
  ...
  const ok = window.confirm(`确定删除“${p.name}”（person_id=${p.person_id}）吗？\n这会同时删除该人的所有 embeddings。`)
  if (!ok) return

  deletingId.value = p.person_id
  try {
    const resp = await fetch(`${API_BASE}/people/${encodeURIComponent(p.person_id)}`, {
      method: 'DELETE',
    })
    const data = await resp.json().catch(() => ({}))
    if (!resp.ok) {
      error.value = data?.detail ? String(data.detail) : `删除失败：HTTP ${resp.status}`
      return
    }
    toast.value = `已删除：${p.name}（embeddings_deleted=${data?.embeddings_deleted ?? 0}）`
    await refresh()
  } catch (e) {
    error.value = e instanceof Error ? e.message : String(e)
  } finally {
...
<template>
  <section class="page">
    <header class="header">
      <div>
        <h1>已标注人员列表</h1>
        <p class="sub">从后端 <code>/people</code> 拉取当前已录入的名字与 embedding 统计。</p>
      </div>
      <div class="actions">
    ...
    <div class="summary" v-else>
      <div class="pill">人数：<b>{{ total }}</b></div>
      <div class="pill">embedding 总数：<b>{{ totalEmbeddings }}</b></div>
      <div class="pill muted">后端：<code>{{ API_BASE }}</code></div>
    </div>
      ...
      <table class="table">
        ...
        <tbody>
          <tr v-if="!loading && items.length === 0">
            <td colspan="6" class="empty">暂无数据。请先去“录入”页面添加人脸。</td>
          </tr>
          <tr v-for="p in items" :key="p.person_id">
            ...
            <td class="ops">
              <button class="btn danger" :disabled="deletingId === p.person_id" @click="deletePerson(p)">
                {{ deletingId === p.person_id ? '删除中…' : '删除' }}
              </button>
            </td>
          </tr>
        </tbody>
